...
class DbClient:
    ...
    def init_vault(self, addr, token, namespace, path, key_name, transform_path, transform_masking_path, ssn_role, ccn_role, dob_role):
    ...
    def encode_dob(self, value):
        try:
            # transform not available in hvac, raw api call
            url = self.vault_client.url + "/v1/" + self.transform_masking_mount_point + "/encode/" + self.dob_role
            payload = "{\n  \"value\": \"" + value + "\",\n  \"transformation\": \"" + self.dob_role + "\"\n}"
            headers = {
                'X-Vault-Token': self.vault_client.token,
                'X-Vault-Namespace': self.namespace,
                'Content-Type': "application/json",
            ...
            response = requests.request("POST", url, data=payload, headers=headers) 
            #response = requests.request("POST", url, payload, headers=headers) 
            logger.debug('Response: {}'.format(response.text))
            return response.json()['data']['encoded_value']
        except Exception as e:
            logger.error('There was an error encrypting the data: {}'.format(e))
        ...
        for row in cursor:
            try:
                ...
                if self.vault_client is not None and not raw:
                     r['Product_Code'] = self.decode_ssn(r['Product_Code'])
                     r['Company_ID'] = self.decrypt(r['Company_ID'])
                     r['Grade_Code'] = self.decrypt(r['Grade_Code'])
                    # r['Date_of_Birth'] = self.decode_dob(r['Date_of_Birth'])
                results.append(r)
            except Exception as e:
    ...
    def get_customer_record(self, id):
        statement = 'SELECT * FROM `customers` WHERE Company_ID = {}'.format(id)
        cursor = self.conn.cursor()
        self._execute_sql(statement, cursor)
        results = []
        for row in cursor:
    ...
    def insert_customer_record(self, record):
        if self.vault_client is None:
            statement = '''INSERT INTO `customers` (`Company_ID`, `First_Name`, `Last_Name`,  `Full_Name`,  `Date_of_Birth`, `Person_Type`, `Designation_Code`,  `Grade_Code`,`Product_Code`,`Reporting_Manager_ID`) 
                            VALUES  (  "{}","{}","{}", "{}", "{}", "{}", "{}", "{}", "{}", "{}");'''.format(record['Company_ID'], record['First_Name'], record['Last_Name'], record['Full_Name'], record['Date_of_Birth'], record['Person_Type'], record['Designation_Code'], record['Grade_Code'],record['Product_Code'], record['Reporting_Manager_ID'] )
        else:
            statement = '''INSERT INTO `customers` ( `Company_ID`,`First_Name`, `Last_Name`,  `Full_Name`,  `Date_of_Birth`, `Person_Type`, `Designation_Code`,  `Grade_Code`,`Product_Code`,`Reporting_Manager_ID`) 
                            VALUES  (  "{}","{}","{}", "{}", "{}", "{}", "{}", "{}", "{}", "{}");'''.format(self.encrypt(record['Company_ID']), record['First_Name'], record['Last_Name'], record['Full_Name'], self.encode_ccn(record['Date_of_Birth']), record['Person_Type'], record['Designation_Code'], self.encrypt(record['Grade_Code']),self.encode_ssn(record['Product_Code']),self.encode_dob(record['Reporting_Manager_ID']) )
    ...
    def update_customer_record(self, record):
        if self.vault_client is None:
            ...
            statement = '''UPDATE `customers`
                       SET First_Name  = "{}", Last_Name  = "{}", Full_Name  = "{}", Employee_Status  = "{}", Gender  = "{}", Date_of_Birth  = "{}", Personal_Email_ID  = "{}", Person_Type  = "{}", Designation_Code  = "{}", Grade_Code  = "{}", Location_Code  = "{}", Cost_Centre  = "{}", Profit_Centre  = "{}", Business_Profit_Centre  = "{}", Final_Business_Profit_Centre  = "{}", Gender  = "{}", Product_Code  = "{}", Reporting_Manager_ID  = "{}", Notice_Period  = "{}", Retirement_Date  = "{}", Office_Email_ID  = "{}", Resignation_Type  = "{}", Date_of_Resignation  = "{}", Date_of_Leaving  = "{}", Employee_Reason_for_Leaving  = "{}", Manager_Reason_for_Leaving  = "{}",  HR_Reason_for_Leaving  = "{}", Resignation_Status  = "{}"
                        WHERE Company_ID = {};'''.format(self.encrypt(record['Company_ID'], record['First_Name'], record['Last_Name'], record['Full_Name'], record['Employee_Status'], record['Gender'], record['Date_of_Birth'], record['Personal_Email_ID'], record['Person_Type'], record['Designation_Code'], self.encrypt(record['Grade_Code']), record['Location_Code'], record['Cost_Centre'], record['Profit_Centre'], record['Business_Profit_Centre'], record['Final_Business_Profit_Centre'], record['Gender'], self.encode_ssn(record['Product_Code']), self.encode_ccn(record['Reporting_Manager_ID']), record['Notice_Period'], record['Retirement_Date'], record['Office_Email_ID'], record['Resignation_Type'], record['Date_of_Resignation'], record['Date_of_Leaving'], record['Employee_Reason_for_Leaving'], record['Manager_Reason_for_Leaving'],  record['HR_Reason_for_Leaving'], record['Resignation_Status']))
        logger.debug('Sql Statement: {}'.format(statement))
        cursor = self.conn.cursor()
        self._execute_sql(statement, cursor)
        self.conn.commit()
