...
def get_customer():
    global dbc
    cust_no = request.args.get('Company_ID')
    if not cust_no:
  ...
  try:
    ...
    if conf.has_section('VAULT'):
      if conf['VAULT']['Enabled'].lower() == 'true':
        ...
        dbc.init_vault(addr=conf['VAULT']['Address'], token=conf['VAULT']['Token'], namespace=conf['VAULT']['Namespace'], path=conf['VAULT']['KeyPath'], key_name=conf['VAULT']['KeyName'], transform_path=conf['VAULT']['TransformPath'], ssn_role=conf['VAULT']['SSNRole'], transform_masking_path=conf['VAULT']['TransformMaskingPath'], ccn_role=conf['VAULT']['CCNRole'],dob_role=conf['VAULT']['CCNRole'])
